...
$(document).ready(function() {
    ...
    $.ajax({
        type: 'post',
        url: '../getCourseFinalTest',
        dataType: 'json',
        data: {
            course_id: course_id
        },
        success: function(f_test) {
            final_test = f_test;
            if (final_test[0].question_text.indexOf("auto_")==0) {
    ...
    $("#theme-selector").change(function () {
        saveTheme();
        var theme_t = $(this).val().split('-');
        var theme_st = theme_t[1];
        theme_t = theme_t[0];
        cur_t = parseInt(theme_t);
        cur_st = parseInt(theme_st);
        $("#creating-theme").css('position', 'relative').animate({
            opacity: '0',
            left: '-=1000px'
...
function prevTheme() {
    ...
    if (saveTheme()) {
        $("#creating-theme").promise().done(function () {
            if (cur_t == 0 && cur_st == 0) return;
            if (cur_st == 0) {
                cur_t--;
...
function newQuestion() {
    if (saveQuestion()) {
        ...
        $("#third_step_form").css('position', 'relative').animate({
            opacity: '0',
            left: '-=1000px'
        }, 400, 'easeInQuint', function () {
...
function finishCourse() {
    if (saveTheme()) {
        ...
        if (final_test.length < 1) final_test = undefined;
...
function loadThemeSelector() {
    ...
    for (var i = 0; i < themes.length; i++) {
        for (var j = 0; j < themes[i].length; j++) {
            if (j == 0) {
                if (i == cur_t && j == cur_st) selector.append("<option selected value='" + i + "-" + j + "'>" + (i + 1) + " " + themes[i][j]['theme_name'] + "</option>");
                else selector.append("<option value='" + i + "-" + j + "'>" + (i + 1) + " " + themes[i][j]['theme_name'] + "</option>");
            }
            else {
                if (i == cur_t && j == cur_st) selector.append("<option selected value='" + i + "-" + j + "'>" + (i + 1) + "." + j + " " + themes[i][j]['theme_name'] + "</option>");
                else selector.append("<option value='" + i + "-" + j + "'>" + (i + 1) + "." + j + " " + themes[i][j]['theme_name'] + "</option>");
            }
        }
    }
}
...
function setManualFinal() {
    ...
    if (final_test.length > 0 && final_test[0].question_text.indexOf("auto_")==0) {
        for (var i=0; i < final_test.length; i++) {
            final_test[i].question_text = final_test[i].question_text.split('_')[3];
        }
        loadQuestion();
    }
}